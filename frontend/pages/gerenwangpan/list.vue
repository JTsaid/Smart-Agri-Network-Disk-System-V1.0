...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 0","alignItems":"center","background":"none","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","fontSize":"16px","display":"inline-block"}'>文件名称</div>
        <el-input v-model="formSearch.wenjianmingcheng" placeholder="文件名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","fontSize":"16px","display":"inline-block"}'>文件分类</div>
        <el-select v-model="formSearch.wenjianfenlei" placeholder="请选择文件分类" :clearable="true">
          <el-option v-for="(item, index) in wenjianfenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('gerenwangpan','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#1e8892","width":"auto","fontSize":"16px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/gerenwangpanAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"16px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
  ...
  export default {
    ...
    created() {
      ...
      this.baseUrl = this.$config.baseUrl;
	    this.wenjianfenleiOptions = '图片,文档,视频,音频,其他文件'.split(',');
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.wenjianmingcheng != '') searchWhere.wenjianmingcheng = '%' + this.formSearch.wenjianmingcheng + '%';
        if (this.formSearch.wenjianfenlei != '') searchWhere.wenjianfenlei = this.formSearch.wenjianfenlei;
        params['sort'] = 'shangchuanshijian';
        params['order'] = 'desc';
        this.$http.get('gerenwangpan/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/gerenwangpanDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
